...
const defaultGraphQLMainnet = "https://graphql.panl.org/";
...
const createConfig = (): Config => {
  ...
  const defaultSubmitURI =
    network === "mainnet" ? defaultSubmitURIMainnet : defaultSubmitURITestnet;
  const submitEnv = process.env.NEXT_PUBLIC_SUBMIT;
  const submitURI = submitEnv ? submitEnv.split(";") : defaultSubmitURI;
  const defaultSMASH =
    network === "mainnet" ? defaultSMASHMainnet : defaultSMASHTestnet;
...
const ConfigContext = createContext<[Config, (x: Config) => void]>([
  defaultConfig,
  (_) => {},
]);
...
const defaultGraphQLURI =
  ...
  (parseNetwork(process.env.NEXT_PUBLIC_NETWORK ?? "mainnet") === "mainnet"
    ? defaultGraphQLMainnet
    : defaultGraphQLTestnet);
